...
require 'models.WeightNormalizedFullConvolution'
...
local WNFC = nn.WeightNormalizedFullConvolution
...
function Generator.input_block(g_config)
    ...
    if flag_wn then input_block:add(WNFC(nz, ngf, 4, 4):init('weight', nninit.kaiming, {gain = {'lrelu', leakiness = 0.2}}))
    ...
    if flag_wn then input_block:add(WNFC(ngf, ngf, 3, 3, 1, 1, 1, 1):init('weight', nninit.kaiming, {gain = {'lrelu', leakiness = 0.2}}))
...
function Generator.intermediate_block(resl, g_config)
    ...
    if halving then
        if flag_wn then inter_block:add(WNFC(ndim*2, ndim, 3, 3, 1, 1, 1, 1):init('weight', nninit.kaiming, {gain = {'lrelu', leakiness = 0.2}}))
        ...
        if flag_wn then inter_block:add(WNFC(ndim, ndim, 3, 3, 1, 1, 1, 1):init('weight', nninit.kaiming, {gain = {'lrelu', leakiness = 0.2}}))
...
function Generator.to_rgb_block(ndim, g_config)
    ...
    if flag_wn then to_rgb_block:add(WNFC(ndim, nc, 1, 1):init('weight', nninit.kaiming, {gain = {'lrelu', leakiness = 0.2}}))
