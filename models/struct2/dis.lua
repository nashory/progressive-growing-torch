...
require 'models.WeightNormalizedConvolution'
require 'models.WeightNormalizedLinear'
...
local WNC = nn.WeightNormalizedConvolution
local WNLinear = nn.WeightNormalizedLinear
...
function Discrim.output_block(d_config)
    ...
    -- conv1 (3x3)
    if flag_wn then output_block:add(WNC(ndf+1, ndf, 3, 3, 1, 1, 1, 1):init('weight', nninit.kaiming, {gain = {'lrelu', leakiness = 0.2}}))
    else output_block:add(SConv(ndf+1, ndf, 3, 3, 1, 1, 1, 1):init('weight', nninit.kaiming, {gain = {'lrelu', leakiness = 0.2}})) end
    if flag_lrelu then output_block:add(nn.LeakyReLU(0.2,true)) else output_block:add(nn.ReLU(true)) end
    -- conv2 (4x4)
    if flag_wn then output_block:add(WNC(ndf, ndf, 4, 4):init('weight', nninit.kaiming, {gain = {'lrelu', leakiness = 0.2}}))
    ...
    output_block:add(nn.WNLinear(ndf, 1))
...
function Discrim.intermediate_block(resl, d_config)
    ...
    if halving then
        if flag_wn then inter_block:add(WNC(ndim, ndim, 3, 3, 1, 1, 1, 1):init('weight', nninit.kaiming, {gain = {'lrelu', leakiness = 0.2}}))
        else inter_block:add(SConv(ndim, ndim, 3, 3, 1, 1, 1, 1):init('weight', nninit.kaiming, {gain = {'lrelu', leakiness = 0.2}})) end
        if flag_lrelu then inter_block:add(nn.LeakyReLU(0.2,true)) else inter_block:add(nn.ReLU(true)) end

        if flag_wn then inter_block:add(WNC(ndim, ndim*2, 3, 3, 1, 1, 1, 1):init('weight', nninit.kaiming, {gain = {'lrelu', leakiness = 0.2}}))
        else inter_block:add(SConv(ndim, ndim*2, 3, 3, 1, 1, 1, 1):init('weight', nninit.kaiming, {gain = {'lrelu', leakiness = 0.2}})) end
        if flag_lrelu then inter_block:add(nn.LeakyReLU(0.2,true)) else inter_block:add(nn.ReLU(true)) end
...
function Discrim.from_rgb_block(ndim, d_config)
    ...
    if flag_wn then from_rgb_block:add(WNC(nc, ndim, 1, 1):init('weight', nninit.kaiming, {gain = {'lrelu', leakiness = 0.2}}))
    else from_rgb_block:add(SConv(nc, ndim, 1, 1):init('weight', nninit.kaiming, {gain = {'lrelu', leakiness = 0.2}})) end
