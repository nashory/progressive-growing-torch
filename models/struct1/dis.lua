...
function Discrim.output_block(d_config)
    ...
    -- conv1 (3x3)
    if flag_wn then output_block:add(WNC(ndf+1, ndf, 3, 3, 1, 1, 1, 1))
    ...
    -- conv2 (4x4)
    if flag_wn then output_block:add(WNC(ndf, ndf, 4, 4))
    ...
    output_block:add(WNLinear(ndf, 1))
...
function Discrim.intermediate_block(resl, d_config)
    ...
    if halving then
        if flag_wn then inter_block:add(WNC(ndim, ndim, 3, 3, 1, 1, 1, 1))
        ...
        if flag_wn then inter_block:add(WNC(ndim, ndim*2, 3, 3, 1, 1, 1, 1))
...
function Discrim.from_rgb_block(ndim, d_config)
    ...
    if flag_wn then from_rgb_block:add(WNC(nc, ndim, 1, 1))
