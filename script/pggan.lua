...
function PGGAN:__init(model, criterion, opt, optimstate, config)
    ...
    self.batchSize = batch_table[math.pow(2, self.resl)]
...
function PGGAN:ResolutionScheduler()
    ...
    if (self.kimgs%TICK) < (prev_kimgs%TICK) then
        ...
        if math.floor(self.resl) ~= prev_resl then
            ...
            self.loader.l_config.loadSize = math.pow(2, math.floor(resl))
            self.loader.l_config.sampleSize = math.pow(2, math.floor(resl))
            self.loader:renew(self.loader.l_config)
...
PGGAN['fDx'] = function(self, x)
    ...
    local d_errD_real = self.crit_adv:backward(f_x:cuda(), self.label:cuda())
...
PGGAN['fGx'] = function(self, x)
    ...
    local d_fx_tilde = self.gen:updateGradInput(self.x_tilde:cuda(), d_errG:cuda())
...
function PGGAN:train(epoch, loader)
    ...
    for iter = 1, TICK*opt.total_tick do
        -- calculate iteration.
        globalIter = globalIter + 1
        if globalIter%(math.ceil(self.loader:size()))==0 then epoch = epoch + 1 end
        ...
        -- logging
        local log_msg = string.format('[E:%d][T:%d][%6d/%6d] errD(real): %.3f | errD(fake): %.3f | errG: %.3f | [Res:%4d][Trn:%.1f%%][Elp:%.4f]',
                                        epoch, self.globalTick, globalIter%(math.ceil(self.loader:size())), self.loader:size(), 
                                        errD.real, errD.fake, errG, math.pow(2, math.pow(self.resl)), self.complete, tm:time().real)
        print(log_msg)
